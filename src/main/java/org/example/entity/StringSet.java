...
public class StringSet {
 ...
 "is held in the British Library. Macmillan published a facsimile of the manuscript in 1886.";
        ...
        Set<String> filtered = uniqueWords.stream().filter(s -> !s.isBlank()).collect(Collectors.toSet());
        TreeSet<String> result = new TreeSet<>(filtered);
        System.out.println("after treeset: "+ result.size());
